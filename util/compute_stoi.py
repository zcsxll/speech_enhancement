...
def stoi(x, y, fs_signal=16000):
    ...
    params:
        x: clean speech
        y: processed speech, whose length should be same with x
    returns:
        stoi scores
    '''
    x = copy.deepcopy(x)
    ...
    x_hat = stdft(x, N_frame, N_frame // 2,
                  K)  # apply short-time DFT to clean speech
    y_hat = stdft(y, N_frame, N_frame // 2,
                  K)  # apply short-time DFT to processed speech
    ...
    X = np.zeros(
        (J, nframes
         ))  # init memory for clean speech 1/3 octave band TF-representation
    Y = np.zeros(
        (J, nframes)
    ...
    for i in range(nframes):
        X[:, i] = np.sqrt(
            np.dot(H,
                   np.abs(x_hat[:, i])**
                   2))  # apply 1/3 octave bands as described in Eq.(1) [1]
    ...
    d_interm = np.zeros(
        (J, nframes - N +
         1))  # init memory for intermediate intelligibility measure
    c = 10**(-Beta / 20.0)  # constant for clipping procedure

    for m in range(N, nframes + 1):
        X_seg = X[:,
                  m - N:m]  # region with length N of clean TF-units for all j
        Y_seg = Y[:, m -
                  N:m]  # region with length N of processed TF-units for all j
        alpha = np.sqrt(
            np.sum(X_seg**2, axis=1) / np.sum(Y_seg**2, axis=1)
        )  # obtain scale factor for normalizing processed TF-region for all j
        aY_seg = Y_seg * np.tile(np.array(
            [alpha]).T, (1, N))  # obtain \alpha*Y_j(n) from Eq.(2) [1]
        for j in range(J):
            Y_prime = np.min(np.vstack(
                (aY_seg[j, :], X_seg[j, :] + X_seg[j, :] * c)),
                             axis=0)  # apply clipping from Eq.(3)
            d_interm[j, m - N] = taa_corr(
                X_seg[j, :],
                Y_prime[:])  # obtain correlation coeffecient from Eq.(4) [1]
...
def removeSilentFrames(x, y, Range, N, K):
    ...
    for j in range(len(frames)):
        ...
        msk[j] = 20.0 * np.log10(
            numpy.linalg.norm(x[frames[j]:frames[j] + N] * w) / np.sqrt(N) +
            1e-100)
    ...
    for j in range(len(frames)):
        if msk[j]:
            x_sil[frames[count]:frames[count] +
                  N] += x[frames[j]:frames[j] + N] * w
            y_sil[frames[count]:frames[count] +
                  N] += y[frames[j]:frames[j] + N] * w
            count += 1
